...
T_im1 = maketform('projective',im1_X);
...
T_im2 = maketform('projective',im2_X);
...
combpart = uint16(repmat(mask_im1 .* mask_im2,[1 1]));
combmask = uint16(combpart > 0);

stitched_image = zeros(size(im1,1),size(im1,2),size(im1,3));
for i = 1:size(im1,3)
    stitched_image(:,:,i) = repmat(im1part,[1 1]) .* uint16(im1(:,:,i)) + repmat(im2part,[1 1]) .* uint16(im2(:,:,i));
    stitched_image(:,:,i) = uint16(stitched_image(:,:,i)) + ( combpart .* uint16(im1(:,:,i)) + combmask .* uint16(im2(:,:,i)) ) ./ (combpart + uint16(ones(size(combpart,1),size(combpart,2))));
end
